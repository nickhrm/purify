...
def analyze_fidelity_dependency(
    case_study_id, coherence_time, lambdas, output_file="fidelity_analysis.csv"
):
    ...
    print(
        f"Daten für T_coh={coherence_time} (Lambdas={lambdas}) an {output_file} "
        f"angehängt. (Insgesamt {len(df)} Zeilen geschrieben.)"
    )
